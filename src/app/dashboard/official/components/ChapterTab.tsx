...
import { Eye, FileText, Download, CheckCircle, X, Check, Mail } from "lucide-react";
import SendInstitutionMailModal, { MailRecipientInfo } from "./SendInstitutionMailModal";

export interface ChapterReq {
  id: string;
  aishe: string;
  name: string;
  state: string;
...
interface Props {
  requests: ChapterReq[];
  onVerify: (id: string, action: "approved" | "rejected") => void;
  onNotify?: (msg: string) => void;
}

export default function ChapterTab({ requests, onVerify, onNotify }: Props) {
  ...
  const handleOpenMail = (r: ChapterReq) => {
    setMailRecipient({
      recipientEmail: r.spocEmail || "",
      recipientName: r.spoc || "Institutional Coordinator",
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-2 sm:gap-0">
        <div>
          ...
          <p className="text-[11px] text-zinc-500 mt-0.5">
            Audit and process affiliation applications from STEM institutions and dispatch official communications.
          </p>
        </div>
        <div className="text-left sm:text-right text-[10px] text-zinc-500 self-start sm:self-auto">
          <p>
            Total: <strong className="text-zinc-800">{requests.length}</strong> &nbsp;|&nbsp; Pending:{" "}
            <strong className="text-red-700">{pending}</strong>
          </p>
          <p>
            Approved: <strong className="text-emerald-700">{requests.filter((r) => r.status === "approved").length}</strong>
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-zinc-200">
        <div className="px-4 py-2.5 border-b border-zinc-200 bg-zinc-50 flex items-center justify-between">
          <span className="text-xs font-bold text-zinc-800 uppercase tracking-wider">
            Affiliation Applications Queue — Form NCIE-AFF-2026
          </span>
          <span className="text-[10px] text-zinc-400">Click &ldquo;Audit&rdquo; to review or &ldquo;Mail&rdquo; to email SPOC</span>
        </div>
                ...
                requests.map((r, i) => (
                  <tr key={r.id} className={`${i % 2 === 0 ? "bg-white" : "bg-zinc-50/40"} hover:bg-emerald-50/30 transition-colors`}>
                    ...
                    <td className="px-4 py-2.5">
                      <p className="font-semibold text-zinc-800">{r.spoc}</p>
                      <p className="text-[10px] text-zinc-400 font-mono">{r.spocEmail}</p>
                    </td>
                    <td className="px-4 py-2.5 text-center">
                      {r.status === "pending" && (
                        <span className="text-[9px] font-bold px-2 py-0.5 bg-amber-50 text-amber-800 border border-amber-300 uppercase">
                          Pending
                        </span>
                      )}
                      {r.status === "approved" && (
                        <span className="text-[9px] font-bold px-2 py-0.5 bg-green-50 text-green-800 border border-green-300 uppercase">
                          Approved
                        </span>
                      )}
                      {r.status === "rejected" && (
                        <span className="text-[9px] font-bold px-2 py-0.5 bg-red-50 text-red-800 border border-red-300 uppercase">
                          Rejected
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-2.5 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        <button
                          ...
                          className="bg-[#0D6B4F] hover:bg-[#0a5840] text-white text-[10px] font-bold px-2.5 py-1 cursor-pointer inline-flex items-center gap-1 shadow-2xs rounded"
                          title="Audit Documents & Process"
                        >
                          <Eye className="w-3 h-3" /> Audit
                        </button>
                        <button
                          onClick={() => handleOpenMail(r)}
                          className="bg-emerald-50 hover:bg-emerald-100 text-[#0D6B4F] border border-emerald-300 text-[10px] font-bold px-2.5 py-1 cursor-pointer inline-flex items-center gap-1 rounded transition-colors"
                          title="Dispatch Email to Institution SPOC"
                        >
          ...
          <div className="bg-white border border-zinc-300 w-full max-w-xl shadow-2xl">
            <div className="bg-[#0D6B4F] text-white px-5 py-3 flex justify-between items-center">
              <div>
                <p className="text-[9px] font-bold text-emerald-200 uppercase tracking-widest">
                  Document Audit — {selected.aishe}
                </p>
                <p className="text-sm font-bold">{selected.name}</p>
              </div>
              <button
                onClick={() => setSelected(null)}
                className="text-white/70 hover:text-white text-xs border border-white/30 px-2 py-1 cursor-pointer flex items-center gap-1"
              >
            ...
            <div className="p-5 space-y-4">
              <table className="w-full text-xs border border-zinc-200">
                <tbody className="divide-y divide-zinc-100">
                  {[
                    ["AISHE Code", selected.aishe],
                    ["Institution", selected.name],
                    ["Type", selected.type],
                    ["State", selected.state],
                    ["Nodal SPOC", selected.spoc],
                    ["SPOC Email", selected.spocEmail],
                  ].map(([k, v]) => (
                        ...
                        <div className="border border-zinc-200 p-2.5 flex items-center justify-between bg-zinc-50 rounded">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-zinc-700">
                              Accreditation / Incorporation Cert
                            </span>
                          </div>
                          <a
                            href={docObj.idCard}
              ...
              <div className="text-[10px] bg-amber-50 border border-amber-200 px-3 py-2 text-zinc-500">
                Verify AICTE/UGC affiliation document authenticity before approving. You may also dispatch an email query or approval notice directly to the SPOC.
              </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
                <button
                  ...
                  className="bg-emerald-50 hover:bg-emerald-100 text-[#0D6B4F] border border-emerald-300 text-xs font-bold px-3 py-2 cursor-pointer transition-all flex items-center gap-1.5 rounded"
                >
                  <Mail className="w-3.5 h-3.5" /> Dispatch Mail to SPOC
                </button>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="bg-white hover:bg-red-50 text-red-700 text-xs font-bold px-3 py-2 border border-red-400 cursor-pointer transition-all flex items-center gap-1.5 rounded"
                  >
                    <X className="w-3.5 h-3.5" /> Reject Application
                  </button>
                  <button
                    onClick={() => {
                      onVerify(selected.id, "approved");
                      setSelected(null);
                    }}
                    className="bg-[#0D6B4F] hover:bg-[#0a5840] text-white text-xs font-bold px-3 py-2 border border-[#0D6B4F] cursor-pointer transition-all flex items-center gap-1.5 rounded"
                  >
